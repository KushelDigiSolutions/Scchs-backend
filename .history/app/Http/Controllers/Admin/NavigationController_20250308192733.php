...
class NavigationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            'title' => 'required',
            'storeObjectItems' => 'required', // At least one item required
            'storeObjectItems.*' => 'required', // Each item must be a string
            'is_active' => 'required|boolean', // Required boolean (1 or 0)
        ], [
...
{
    $request->validate([
        'status' => 'required|in:1,0', // ENUM Validation
    ]);

    $navigation = Navigation::find($id);

    if (!$navigation) {
        return back()->with('error', 'Navigation not found!');
    }

    // Update is_published status
    $navigation->is_published = $request->status;
    $navigation->save();

    // Success message based on status
    $message = ($request->status == '1') ? 'Navigation Activated' : 'Navigation Deactivated';
