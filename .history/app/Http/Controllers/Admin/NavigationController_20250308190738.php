    ...
    public function store(Request $request)
{
    // Validate request
    $request->validate([
        'storeObjectItems' => 'required|array|min:1', // Ensures it is an array and has at least one item
    ], [
        'storeObjectItems.required' => 'Item not added. Please add at least one item.',
        'storeObjectItems.array' => 'Invalid format. Items should be an array.',
        'storeObjectItems.min' => 'You must add at least one item.',
    ]);
    ...
    $navigation->items = json_encode($request->storeObjectItems); // Store as JSON if it's an array
